...
filetype () {
  ...
  if [[ "$type" = " empty" ]]; then
    ...
    type=" Zip compressed Jar archive"
       ...
       type=" Excel document"
  elif [[ "$type" = *Zip\ archive* && ("$name" = *.do[ct][xm]) ]]; then
       type=" Microsoft Word 2007+"
  elif [[ "$type" = *Zip\ archive* && ("$name" = *.pp[st][xm]) ]]; then
       type=" Microsoft PowerPoint 2007+"
  elif [[ "$type" = *Zip\ archive* && ("$name" = *.xl[st][xmb]) ]]; then
...
"
    ...
    msg "append $sep to filename to view the raw data"
    identify -verbose "$2"
  elif [[ "$1" = "audio" || "$1" = "video" ]]&& cmd_exist mediainfo; then
    msg "append $sep to filename to view the raw data"
    mediainfo --Full "$2"
  elif [[ "$1" = "audio" ]]; then
